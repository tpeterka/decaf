...
driver_path = os.environ['DECAF_PREFIX'] + '/examples/direct/python/py_linear_3nodes.so'
...
#
#  entire workflow takes 7 procs because of partial overlap

w = nx.DiGraph()
w.add_node('node0', start_proc=0,  nprocs=4, func='node0', path=mod_path)
w.add_node('node1', start_proc=4,  nprocs=2, func='node1', path=mod_path)
w.add_node('node2', start_proc=6,  nprocs=1, func='node2', path=mod_path)
w.add_edge('node0', 'node1', start_proc=0, nprocs=3, func='dflow', path=mod_path, prod_dflow_redist='count', dflow_con_redist='count')
