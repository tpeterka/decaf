...
*******************/
void Reconstruction(int next, int previous)
{
register float kk;
MPI_Status s[4];
MPI_Request r[4];
...
***********/
void Fluxes(int next, int previous)
{
...
extern "C"
{
...

    {
        ...
        *pd = (float)t_current;                    // just assign something, current time step for example
        ...
        if (t_current == 0)
        {
            ...
            globalComm = (*dataflows)[0]->prod_comm_handle(); 
            ...
            mu1 = A3D(LEN+2, HIG+2, DEP+2);
            mu2 = A3D(LEN+2, HIG+2, DEP+2);
            mu3 = A3D(LEN+2, HIG+2, DEP+2);
            mu4 = A3D(LEN+2, HIG+2, DEP+2);
            mu5 = A3D(LEN+2, HIG+2, DEP+2);
        }
        ...
        { // Time stepping
            ...
            Reconstruction(nnext, previous);
            BounCondOnInterfaces();
            Fluxes(nnext, previous);
            Evolution(1);
        ...
        if (!((t_current + 1) % t_interval))
        {
            ...
            {
                multiToFlat(mu1, mu2, mu3, mu4, mu5);
                std::shared_ptr<VectorConstructData<float> > array = make_shared< VectorConstructData<float> >(mu1, (LEN+2)*(HIG+2)*(DEP+2), 1);
            ...
            int this_dataflow = -1)         // index of one dataflow in list of all
    {
        ...
        if (!((t_current + 1) % t_interval))
        {
            ...
            int i, j, k;
            float* uptr = A3D(LEN+2, HIG+2, DEP+2);
            ...
            for (i = 0; i <= LENN; i++)
            {
                for (j = 0; j <= HIGG; j++)
                {
                    for (k = 0; k <= DEPP; k++)
                    {
                        uptr[(i*(HIG+2)*(DEP+2))+(j*(DEP+2))+k] = (float) *it;
                        it++;
                    }
                }
            }
            ...
            checkSDC3(uptr, LEN+2, HIG+2, DEP+2, t_current, 0);
