...
- Pushed datatypes example
- Pushed Comm::num_inputs(), num_outputs(), start_input(), start_output(); see doc/distribute.md
- Fixed MPI unfreed requests
- Fixed nondeterministic results except one partially overlapping case, still todo; see examples in doc/comm.md
- Pushed DOE documents and sent to Lucy
- Working on talk for JLESC Workshop the week after SC; also will reuse slides for January PI meeting
- TODO: Move private documents to svn

## Patrick

- Pushed Cluster 2002 paper
- Invesigating building dataflow graphs using EVPath software
- DFG master shows how data is described
    - similar to my DIY, Decaf datatypes
    - can chase pointers? yes, does this automatically
- master defines the dataflow graph
- clients connect to it
- EVPath is the top layer of set of GT software, see Jay's individual doc for links

## Jay

- What is lacking in EVpath
    - resilience
    - types done externally or generically, currently strongly typed that way that decaf, diy are
    - pipelining
    - Tom: automatic buffering combined with direct memory and network coupling

## Florin

- Working on breaking up datatypes
- Developing method for launching parallel workflows

## Lokman

- Discussed differences between Decaf and other tools, in particular Damaris
    - flexible communicator assignment
    - resilience
    - a dataflow in between producer and consumer
    - M:N, pipelining
    - automatic buffering
- Decoupling dataflow from deployment decision
- API for deployment
- Patrick: Classes for common dataflow patterns? (Tom: good idea)
- Wrappers for datatypes

## Hadrien

- Pipeline for HACC -> tess -> dense
- Comparing with TESS-based with CIC-based density estimation (as an auxiliary method)
    - large differences, on the order of the values
- Resampling CIC results in excessive blurring
- Use a different auxiliary method
    - SPH?
    - Lower resolution tess?
    - Followup: Hadrien and Tom discussed an adaptive CIC (like SPH but with a cube-shaped window and volume-fraction kernel function); Hadrien will implement and try this

## Guillaume

- Little room for theoretical publishable novelty
- But not convinced that the combination of the entire model is not publishable in a more applied setting, and regardless, it is useful work.

## Upcoming

- No meeting Thansgiving week, 11/26, next meeting 12/10
- Schedule SC meeting (Tom, Jay, Patrick, Lokman)
    - Wednesday lunch break
    - Tom will send mail
- JLESC workshop talks (Tom, Guillaume, Florin, Lokman)
...
- Python interface
    - Top level (user) interface of decaf is now python, used to configure and launch the workflow
    - Lower level (developer) API is C++
    - See README, examples/direct/python/driver.py, doc/python.md
- JLESC talk
    - Uploaded to wiki/public-docs/ along with other public documents (more coming soon)
- January PI meeting
    - I will prepare these. Handout is done, talk slides will be condensed from JLESC talk, poster and quad chart TBD.
- Think about using python interface to link existing binaries to build a workflow, as well as building an entire single app (as I do now)

## Hadrien

- HACC -> tess -> dense pipeline
    - CIC too noisy
    - ACIC better, adaptive size window
- Formulate analytical model for the noise (discuss with LANS?)
- Working on injecting soft error bit flips
    - There is no good model for bit flips or other errors that actually happen
    - Randomly perturb particle positions and integer indices instead

## Lokman

- Splitting datatype
    - wrote functions for splitting datatypes and data
    - still need to write functions for merging datatypes and data
    - working the datatypes example, currently splits datatypes but does not yet move them from producer to consumer and recombine (todo)
- Workflow definitions
    - as FlowVR, Dispel4py
    - Tom's python interface is the mechanism; define the workflow there

## Florin

- benchmarks
- working on last bugs
- January HPDC paper target

## Upcoming

- No meeting 12/24, next meeting 1/7/15
