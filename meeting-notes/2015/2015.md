...
- MPMD mode in mpi
    - total number of procs is n_procs1 + n_procs2 + ... (disjoint in COMM_WORLD)
```
mpiexec -n <n_procs1> <app1> <args2> : -n <n_procs2> <app2> <args2> ...
```
...
- Next meeting 3/11/15 (in 3 weeks time because of travel)
...
- posted FlowVR paper in related work
- reviewed lammps example

## Jay

- EVPath deployments on order of 10K nodes so far
- over portals native infiniband, sits on NTTI
- Matt Wolf adding in more general scripting engine (Lua) into EVPath
    - generic aggregators
- Extracting related work on workflow operators

## Patrick

- Example of deployment of graphs in EVPath
- Preparing for 2 students from Matt Wolf @ GTech (physicists turned CS; one working on decaf and one on Hobbes)

## Hadrien

- Injection models difficult to generate
- Certification, validation of the result
- Modeling prediction of expected results as a function of number of input points, output grid resolution, shape of the distribution
- Using a machine learning approach, generating derivatives using empirical tests

## Florin

- New implementation of collective I/O method
- High fragmentation causes a problem with existing implementations
- Have a solution to constrain the memory buffers, but still need to orchestrate the network calls for nonblocking I/O
